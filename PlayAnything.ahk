...
;  2021/4/27		Change still image duration : 2 sec -> 10 sec
;  2021/4/30		Add filename for youtube download (Some title contains illegal character for filename)
;  2021/5/3		Auto numbering file name in playlist
;  2021/5/17		Improve mediainfo.ahk  (CCTV Audio stream, H.265 avi stream)
;						Improve playlist remaining time calculation (title, still picture...)
;						Report Error on ffmpeg immediate exit
; 2021/5/28		Improve title playback (replace special character)
; 2021/7/15		Add apad option, Add 1 second extra curation for media (safe ffmpeg decklink output)
; 2021/7/19 		Fix Param_processor.ahk for 16 channel audio
; 2022/6/15     Improve mpv low latency parameters
; 2022/8/20		Store, show Recent 20 played list
;
...
I_Icon := A_ScriptDir . "\bin\Movie_Player.ico"
IfExist, %I_Icon%
...
Gui, add, Button, xp  yp+50 w100 h30 hwndhshowrecent gshowrecent, 최근리스트
Gui, add, Text, xm yp+80 w250 h40 hwndhtc Border 0x200  Center, --:--:--.---
...
Gui, add, Edit, xm yp+40 w340 h30 hwndhtitle vtitle_clip, TITLE
Gui, add, Checkbox, xp+345 yp w60 h30 vtitle_auto, AUTO`r`nTITLE
Gui, add, Button, xp+60  yp  w70 h30 hwndhaddtitle gaddtitle , TITLE ▼
Gui, add, Button, xp+75  yp  w70 h30 gnumbering, NUMBER
...
Gui, add, Button, xp+90 yp  w50 h30 gmovedown , ▼
Gui, add, Button, xp+60 yp  w50 h30 gmoveup , ▲
Gui, add, Button, xp+70  yp  w100 h30 glvdelete , Delete
Gui, add, Edit, xm yp+40 w420 h30 vytburl, Youtube URL
Gui, add, Button, xp+440 yp w100 h30 gytbdownload, Download
Gui, add, Edit, xm yp+40 w420 h30 vytbtitle, YOUTUBE SAVE FILENAME
Gui, add, text, xp+430 yp+5 w120 h30, ◀ 다운로드 파일명
Gui, add, StatusBar, hwndhstatus, Application Start
Gui, show,, Play Anything !  by sendust 2022/8/30
...
init_decklink(A_ScriptDir . "\playanything.ini")
...
status.listplay := 0
button_control.btns_disable()
read_info_result(media)
return

report_source_error(d)
{
	global media
	fullpath := media.fullpath
	print("ffmpeg reports error  ( RUN time second ==>)  " .  d . " / " . fullpath)
	FileDelete, error_report.txt
	FormatTime, time_now,, yyyy/MM/dd HH:mm.ss
	FileAppend, PlayAnything Error Report    [ %time_now% ] `r`n ---  Error Playing  --- `r`n %fullpath%, error_report.txt
	Run, notepad.exe error_report.txt
}




init_decklink(inifile)
{
	IniRead, init_string, %inifile%, general, init_string , " -d 4 -o 3"
	binary := A_ScriptDir . "\bin\DeviceConfigure.exe"
	runstring = %binary%  %init_string%
	print("Init Decklink with parameter " . runstring)
	RunWait, %runstring%,, Hide UseErrorLevel, pid
	result := ErrorLevel
	print("Init decklink result is " . result)

}

;  //////////// rename function name as [play_media] depends on HD or UHD   ///////////////////////

play_media(media)				;read any  media and sink to decklink			HD Version
{
	global parameter, ffmpeg
	duration := media.mark_out - media.mark_in + 1				; add 1 second for safe Decklink output

	if ((media.width > 0) and !(media.framerate))				; Image (picture) source	(Title has framerate)
		duration := 10

	if (media.filename = "title")											; Title source
		duration := 5

	if (media.filename = "firstframe")
	duration := 2
	...
	rate_option := " -r 30000/1001 "

	IniRead, sdi_preview, playanything.ini, general, sdi_preview,  "rtsp://127.0.0.1:8854/anything1"
	streaming := " -map [vo2] -pix_fmt yuv420p  -r 30000/1001 -c:v mpeg2video -b:v 8000k -an " . duration_option . "   -f rtsp -rtsp_transport udp " . sdi_preview
	IniRead, outputvar, playanything.ini, general, decklink
	param := parameter.get_decklink_filter(media) .  duration_option . rate_option . "  -map [vo1]  -max_muxing_queue_size 1024  -top 1 -pix_fmt uyvy422 -c:a pcm_s16le  -f decklink """ .  outputvar . """" . streaming
	print(param)
	ffmpeg.start(param)
}


play_media_uhd(media)				;read any  media and sink to decklink		UHD Version	, Under DEV......
{
	global parameter, ffmpeg
	duration := media.mark_out - media.mark_in + 1
	...
	rate_option := " -r 60000/1001 "
	rate_option_pvw := " -r 30000/1001"

	IniRead, sdi_preview, playanything.ini, general, sdi_preview,  "rtsp://127.0.0.1:8854/anything1"
	streaming := " -map [vo2] -pix_fmt yuv420p  -c:v mpeg2video -b:v 8000k -an " . duration_option . rate_option_pvw . "   -f rtsp -rtsp_transport udp " . sdi_preview

	IniRead, outputvar, playanything.ini, general, decklink
	param := parameter.get_decklink_filter_uhd(media) .  duration_option . rate_option . " -map [vo1]  -max_muxing_queue_size 1024  -pix_fmt uyvy422 -c:a pcm_s16le  -f decklink """ .  outputvar . """" . streaming
	print(param)
	ffmpeg.start(param)
}


read_info_result(media)				;read any  media and sink to decklink			HD Version
{
	global parameter, ffmpeg
	if InStr(media.fullpath, "<<TITLE>>")
	{
		print("Title action detected")
		text := media.filename
		play_title(text)
	}
	else
...
play_title(text)					; Create title picture and play it
{
	...
	text := RegExReplace(text, "[.!@#$%^&*();:{}\[\]\\/+=|]", "_")				; Replace invalid character (2021/5/28)
	runstring =  -f lavfi -i color=black:size=1920x1080 -filter_complex drawbox=color=white@0.4:y=480:width=iw:height=120:t=fill,drawtext=text="%text%":fontfile="%fontfile%":fontcolor=white:x=20:y=500:fontsize=55 -y -vframes 1 "%outputfile%"
	print(runstring)
	ffmpeg.start_simple(runstring)
	media2 := MEDIA_CLASS
	media2.fullpath := outputfile
	analyse_media(media2)
	play_media(media2)
}
...
create_firstframe(media)
{
	...
	print("Analyse new frame image")
	media_frame1.fullpath := A_ScriptDir . "\firstframe.jpg"			; read first frame still image
	analyse_media(media_frame1)													;  produced still image and play it
	print("Playout Firstframe image")
	read_info_result(media_frame1)
}
...
title_clip := RegExReplace(title_clip, "[\[\]!@#$%^&*():;'""]", "-")			; Replace invalid character
...
numbering:					; Assign prefix number for each file name (added 2021/5/3)
print("Executing file name renumbering ")
Loop, % LV_GetCount()
{
	listv.restore_lv(A_Index, media)			; resotre media information from listview
	if FileExist(media.fullpath)
	{
		media.fullpath := put_prefix(media.fullpath, A_Index)
		listv.modifylist(media, A_Index)
		show_mediainfo(media)
	}
}
print("Complete file renumbering")

return


put_prefix(filepath, nbr)
{
	SplitPath, filepath, outfilename, outdir, outextension, outnamenoext, outdrive
	newnbr := SubStr("00000" . nbr, -3)
	newfilename := newnbr . "_" . outnamenoext
	newfilename_fullpath := outdir . "\" .  newfilename . "." . outextension
	FileMove, %filepath%, %newfilename_fullpath%, 1
	return newfilename_fullpath
}
...
playlist_continue()
{
	global Status, listv, media, button_control, list20

	list20.list_add(media.filename)

	print("Checking playlist continue play")
	if !status.listplay														; normal play mode (not playlist)
	{
		button_control.btns_enable()
		print("This is not a playlist play")
		return
	}
...
show_listrem(listv)				; added 2021/5/17
{
	global hlistrem
	listrem :=  LV_GetCount() . "   " . secondtotc(listv.get_list_rem(0))		; Get entire playlist duration
	GuiControl,, %hlistrem%, %listrem%
}
...
GuiControlGet, ytbtitle
print(youtube.start(ytburl, ytbtitle))
return
...
Loop, parse, A_GuiEvent, "`n", "`r"
{
	...
	analyse_media(media)
	show_mediainfo(media)
	GuiControl,,%htitle%, % media.filename . "." . media.extension
...
MsgBox, , Recent List, % list20.list_show()
...
analyse_media(ByRef media)			; new, 2019/4/3 from mediainfo.dll  ----> Improve 2021/4/7
{
	...
	media.filename := o_mi.getgeneral("FileName")
...
show_ffmpeg_progress(enc_progress)
{
	...
	if (sec < sec_old)				; Start SDI Monitor window (one time)
	{
		...
		runstring := "  --fps=99  --taskbar-progress=no  --force-seekable=no --input-cursor=no --deinterlace --osd-level=3 --osd-fractions  --input-default-bindings=no  --loop-playlist=force  " .  geometry .  title_param   .  "   """ . url . """"
...
class RecentList
{
	...
	__New()
	{
		this.list_recent.push("This is Recent list first line")
	}


	list_add(title)
	{
		this.list_recent.Push(title)
		if (this.list_recent.maxindex() > 20)
		{
			Loop, % this.list_recent.maxindex() - 1
				this.list_recent[A_Index] := this.list_recent[A_Index + 1]
			this.list_recent.Pop()
		}
		return this.list_recent.maxindex()
	}
	...
	{
		text := ""
		for key, value in this.list_recent
			text .= key . " ===> " .  value . "`r`n"
		updatelog(text)
		return text
	}
...
class FFMPEG_CLASS					; FFMPEG Object (Monitor progress UDP, check PID is running)
{
	...
	start(param)
	{
		...
		Run, %runstring%  -progress udp://127.0.0.1:%port% , %A_WorkingDir%\log, Hide, pid
		this.pid := pid
		this.timer_start()
		this.tick_start := A_TickCount
	}
...
class YOUTUBE_DL_CLASS
{
	...
	{
		binary := this.binary
		FormatTime, stamp_time,, yyyy-MM-dd HH-mm-ss
		stamp_time .= "." . A_MSec
		if StrLen(title)
			option_output := " -o """  . title . "_" .  stamp_time """.%(ext)s"
		else
			option_output := ""
		outputfolder := this.folder
		runstring := binary . " """ . url . """ "  .  option_output
		Run, %runstring%, %outputfolder%,, pid
		return runstring
	}
...
class listviewclass
{
	...
	addlist(media)
	{
		...
		fullpath := media.fullpath
		SplitPath, fullpath, outfilename, OutDir, OutExtension, OutNameNoExt, OutDrive
		media_property := media.fullpath . "|" . media.scantype . "|" . media.width . "|" . media.height . "|" . media.audio_format . "|" .  media.codecv . "|" . media.durationframe . "|" . media.framerate . "|" .  media.extension  . "|" .  media.codeca
		LV_Add(, count, OutNameNoExt, media.mark_in, media.mark_out, (media.mark_out - media.mark_in), media_property)
		LV_ModifyCol()

	}

	modifylist(media, number)
	{
		fullpath := media.fullpath
		SplitPath, fullpath, outfilename
		media_property := media.fullpath . "|" . media.scantype . "|" .  media.width . "|"  .  media.height . "|"  . media.audio_format . "|"  . media.codecv . "|"  . media.durationframe . "|"  . media.framerate . "|" . media.extension  . "|" .  media.codeca
		LV_Modify(number , , , outfilename, media.mark_in, media.mark_out, (media.mark_out - media.mark_in), media_property)
		LV_ModifyCol()
	...
	addtitle(text)
	{
		...
		count := % LV_GetCount()
		count += 1
		media_property :=  "<<TITLE>>|PROGRESSIVE|1920|1080|NOAUDIO|PICTURE|1|9999||"
		LV_Add(, count, title_show, 0, 5, 5, media_property)		; change title duration -> from 1 frame to 5 second
		LV_ModifyCol()
	}
	...
	{
		media.reset()
		LV_GetText(outputvar, row, 2)
		media.filename := outputvar
		...
		media.extension := array_split[9]
		media.codeca := array_split[10]
		media.resolution := media.width . "x" . media.height
		printobjectlist(media)
	}
